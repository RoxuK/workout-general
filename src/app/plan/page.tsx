...
export default function PlanPage() {
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className="card">
        <div className="font-display text-xl">{t(plan.name)}</div>
        <p className="mt-1 text-sm text-muted">{t(plan.summary)}</p>
        <div className="mt-3 flex flex-wrap gap-2">
          <span className="chip">{t(plan.structure)}</span>
          <span className="chip">{t(plan.frequency)}</span>
          <span className="chip">{plan.startingWeight} → {plan.targetWeight} kg</span>
        </div>
      </div>
        ...
        {plan.phases.map((f, i) => (
          <div key={i} className="card-2">
            <div className="flex items-center justify-between">
              <div className="font-medium">{t(f.name)}</div>
              <span className="chip">{t("Sem")} {f.weeks}</span>
            </div>
            <p className="mt-1 text-sm text-muted">{t(f.goal)}</p>
      ...
      <div className="card divide-y divide-line p-0">
        {plan.weeklySplit.map((s, i) => (
          <div key={i} className="flex items-center justify-between px-4 py-3">
            <span className="text-sm text-muted">{t(s.day)}</span>
            <span className="text-sm">{t(s.session)}</span>
          </div>
        ))}
      </div>
      <p className="mt-2 text-xs text-muted">{t(plan.splitNote)}</p>
      ...
      <div className="space-y-2">
        {plan.sessions.map((s) => (
          <Link key={s.id} href={`/entreno/${s.id}`} className="card-2 flex items-center justify-between">
            <div>
              <div className="font-display text-lg">{t(s.name)}</div>
              <div className="text-xs text-muted">{t(s.focus)} · {s.exercises.length} {t("ejercicios")}</div>
            </div>
      ...
      <div className="card">
        <div className="mb-2 flex items-center gap-2 text-sm text-accent">
          <Flame size={16} /> {plan.warmup.duration} · {t("obligatorio")}
        </div>
        <ol className="space-y-2">
          {plan.warmup.steps.map((p, i) => (
            <li key={i} className="text-sm">
              <span className="text-ink">{t(p.exercise)}</span>{" "}
              <span className="text-muted">— {t(p.detail)}</span>
            </li>
          ))}
        </ol>
        <p className="mt-3 rounded-xl bg-accent-soft p-3 text-xs text-muted">{t(plan.warmup.note)}</p>
      </div>
      ...
      <div className="space-y-2">
        {plan.progressions.map((p, i) => (
          <div key={i} className="card-2">
            <div className="font-medium text-accent">{t(p.phase)}</div>
